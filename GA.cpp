...
void Draw_Scallop(Scallop &scallop, Mat &dst_image)
{
	...
	for (int i_triangles = 0; i_triangles < TRIANGLES_NUM; i_triangles++)
	{
		...
		begin = new_temp_image.begin<Vec4b>();
		end = new_temp_image.end<Vec4b>();
		while (begin != end)
		{
			(*begin)[0] = 0;
			(*begin)[1] = 0;
...
double Calculate_Fit_Value(Scallop &scallop)
{
	...
	for (int i = 0; i < dst_image.rows; i++)
	{
		for (int j = 0; j < dst_image.cols; j++)
		{
			...
			// 像素差在15以内用l1距离，否则用l2距离，加速收敛，距离归一化到0-1之间
			double p_diff_0 = abs(ideal_point[0] - dst_point[0]) / 255.;
			if (p_diff_0 <= PIXEL_DIFF_THRESH)
				p_diff_0 = pow(p_diff_0, 2);
			double p_diff_1 = abs(ideal_point[1] - dst_point[1]) / 255.;
			if (p_diff_1 <= PIXEL_DIFF_THRESH)
				p_diff_1 = pow(p_diff_1, 2);
			double p_diff_2 = abs(ideal_point[2] - dst_point[2]) / 255.;
			if (p_diff_2 <= PIXEL_DIFF_THRESH)
				p_diff_2 = pow(p_diff_2, 2);
			double p_diff_3 = abs(ideal_point[3] - dst_point[3]) / 255.;
			if (p_diff_3 <= PIXEL_DIFF_THRESH)
				p_diff_3 = pow(p_diff_3, 2);

			fit_result += 1 - (p_diff_0 + p_diff_1 + p_diff_2 + p_diff_3) / 4.;
		}
	}
	fit_result /= (dst_image.rows * dst_image.cols);
...
void Mutation(Scallop *pop)
{
	...
	for (int i = 0; i < POP_NUM; i++)
	{
		...
		if (rand() % 100 <= int(MUT_RATE * 100))
		{
			...
			{
			case 0:
				pop[i].chromosome[gene_num_1].color[0] = rand() % 255;
				break;
			case 1:
				pop[i].chromosome[gene_num_1].color[1] = rand() % 255;
				break;
			case 2:
				pop[i].chromosome[gene_num_1].color[2] = rand() % 255;
				break;
			case 3:
				pop[i].chromosome[gene_num_1].color[3] = rand() % 64; // alpha通道变异范围0-64
				break;
			case 4 ... 6:
				pop[i].chromosome[gene_num_1].point1.x = rand() % ideal_image_length;
				break;
			case 7 ... 9:
				pop[i].chromosome[gene_num_1].point1.y = rand() % ideal_image_length;
				break;
			case 10 ... 12:
				pop[i].chromosome[gene_num_1].point2.x = rand() % ideal_image_length;
				break;
			case 13 ... 15:
				pop[i].chromosome[gene_num_1].point2.y = rand() % ideal_image_length;
				break;
			case 16 ... 18:
				pop[i].chromosome[gene_num_1].point3.x = rand() % ideal_image_length;
				break;
			case 19 ... 21:
				pop[i].chromosome[gene_num_1].point3.y = rand() % ideal_image_length;
				break;
			}
...
void Draw_Max_Scallop(Scallop *pop, int generation)
{
	...
	string str = "/Users/.../Personal/Projects/interest/GA-Firefox-/results/第" + generation_str + "代最优扇贝" + fitness_str + ".png";
...
int main(int argc, char *argv[])
{
	...
	ideal_image = imread("demo/ideal_firefox_128.png", IMREAD_UNCHANGED);
	ideal_image_length = ideal_image.rows;

	// 图像需要做预处理抠图
	// TODO::添加mask，mask外的部分不生成三角形且区域内适应度计算权重降低
